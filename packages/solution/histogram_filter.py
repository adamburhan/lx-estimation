...
def histogram_predict(belief, left_encoder_ticks, right_encoder_ticks, grid_spec, robot_spec, cov_mask):
    ...
    dphi_right = right_encoder_ticks * alpha
    dphi_left = left_encoder_ticks * alpha
    d_right = R * dphi_right
    d_left = R * dphi_left
    d_A = (d_left + d_right)/2
    d_theta = (d_right - d_left)/baseline

    v = d_A * np.sin(grid_spec['phi'] + d_theta)  # replace this with a function that uses the encoder
    w = d_theta  # replace this with a function that uses the encoder
...
def histogram_update(belief, segments, road_spec, grid_spec):
    ...
    if measurement_likelihood is not None:
        ...
        belief = measurement_likelihood * belief / np.sum(measurement_likelihood * belief)
